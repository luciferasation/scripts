...
do
    target=${file%.*}
    if (test -a ./$target.ba) then
        printf "Move %s.ba out of the current directory before running this script." "$target"
        continue
    fi
    if (test -a ./$target.ra) then
        printf "Move %s.ra out of the current directory before running this script." "$target"
        continue
    fi
    ...
    tag+="\n"
    atom=(`head -n $(($num_atom + 2)) $file | tail -n $num_atom | cut -d' ' -f1`)
    awk -v num=$num_atom 'BEGIN { printf("#step time nuclear_kinetic_energy potential_energy total_energy total_angular_momentum\n")} { if(FNR%(num+2)==2) printf("%s %s %s %s %s %s\n", $2, $4, $9, $13, $17, $22)}' $file > $target.ba
    awk 'BEGIN { FS = " " } { if(NF==4) print $0 }' $file | paste -sd "$tag" | awk -f bond_angle_dihedral.awk -v l="${atom[*]}" -v b="$bg" -v a="$ag" -v d="$dg" -v t="$tg" > $target.ra
    pr -mt --separator=" " $target.ba $target.ra | column -t > $target.dat && rm -f $target.ba $target.ra
done
